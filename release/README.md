...
本目录汇集论文 [`../paper/Report.pdf`](../paper/Report.pdf) 报告的终版模型权重与对应的原始（raw，未校准）评测结果。基础 RMSE / MAPE 为生成路径上欧式期权价格相对 10 万条 MC oracle 的误差；新增的 Asian RMSE 使用同一 oracle 的算术平均亚式看涨 payoff；峰度目标 4.60。
